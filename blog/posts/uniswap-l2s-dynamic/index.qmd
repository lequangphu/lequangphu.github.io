...
Not all L2s of Uniswap are equal. Uniswap on Arbitrum may contribute the most volume, but it also has the highest volume concentration in top traders, not to mention having the highest gas. If diversifying sources volume is a concern, and it should be, which L2 should Uniswap put more resources into? The best option, in my opinion, is Polygon, the current no.2 L2, but far behind Arbitrum in recent monthly volume. A few reasons including: (1) low gas, which Uniswap doesn't have control over; (2) having the most loyal traders with the highest week 1 retention rate; (3) having the highest new token listing frequency.