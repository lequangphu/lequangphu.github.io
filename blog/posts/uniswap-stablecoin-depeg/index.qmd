...
vol_line = go.Scatter(x=df_tv['blocktime'], y=df_tv['vol'], name='Volume', line=dict(color=uniswap_color))
...
During depeg, Dexs volume per hour peaked at $1.54B at around 6am Mar 11, 2023. Around that time, USDC and Dai bottomed at around $0.9. Meanwhile, Tether remained peg, and peaked at $1.02.
...
* During depeg:
    + at first, volume on Curve rose faster than on Uniswap (till 1am Mar 11). After that, volume on Uniswap surpassed Curve and peaked when depeged stablecoins made the lowest low. => It seems that [fear/arbitrage opportunities] were realized and extracted earlier on Curve than on Uniswap.
    + Curve saw its share in total Dexs volume increased to 35% from 13%, almost tripled. Meanwhile, Uniswap's share decreased to 61% from 81%.

After depeg, the volume share looks similar to before depeg.
...
* During depeg:
    + Volume in Stable ↔️ Stable Pools rose faster than in Stable ↔️ Non-stable Pools when depeged stablecoins price were making lower lows. Volume in the latter peaked after but higher than volume in the former ($846M vs $830M per hour)
    + When depeged stablecoins price were recovering (to $1), volume in Stable ↔️ Non-stable Pools dropped faster than in Stable ↔️ Stable Pools. => It seems that, not only arbitrage opportunities in Stable ↔️ Non-stable pools were realized later but also extracted faster than in Stable ↔️ Stable pools.
    + Overall, Stable ↔️ Stable accounts for more than half total volume, increased to 52% from 20% before depeg. Meanwhile, Stable ↔️ Non-stable lost its share in total volume to 39% from 58% Before depeg.
...
* During depeg:
    + despite the fear of depegging, USDC was the most swap-to coin, especially when price was making lower lows.
...
After depeg, USDT gained share from USDC in total Dexs volume, increased to 16% from 12%. Meanwhile, USDC's share decreased to 29% from 34%.
...
* During depeg:
    + USDT and WETH saw the most net liquidity in-flow, added $266M and $226M respectively.
    + USDC and Other Coins saw the most net liquidity out-flow, lost $159M and $408M respectively.
...
::: callout-warning
Some Flow charts below might be at odd places. This is a defect when charting with Plotly, which I haven't been able to fix.
:::
...
Before depeg, USDC ↔️ WETH was the biggest flow with $2.7B in both directions, and more than double the second biggest flow, which was WETH ↔️ Other Coins. Meanwhile, USDC ↔️ USDT was the third biggest flow with $480M.

* During depeg:
    + Uniswap saw the rise of Stable ↔️ Stable pools, specifically is the USDC ↔️ USDT flow.
    + USDC ↔️ WETH was still the biggest flow with $8.2B (3x vs Before depeg). However the flow which increased the most was USDC ↔️ USDT with $6.8B (14x vs Before depeg).
    + Other noticable increases were USDT ↔️ WETH: 10x to $3B from $300M; USDC ↔️ DAI: 18x to $1.5B from $80M. After depeg, the flows has been similar to Before depeg.
...
* During depeg:
    ...
    + The main difference was flows to and from DAI, which replaced WETH as the third most volume swap-to and swap-from coin.
    + Total liquidity flow in/out DAI 29x to $2.9B from $100M before depeg.
    
After depeg, WETH regained its third posistion.
...
Since depeg began, bb-a-USDC and bb-a-DAI (Balancer Boosted Aave stable coins) have de-throned USDC in liquidity flow on Other Dexs.
...
* [Queries](https://github.com/lequangphu/lequangphu.github.io/tree/main/blog/posts/uniswap-stablecoin-depeg/resource)