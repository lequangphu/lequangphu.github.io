...
```{python}
df_price = pd.DataFrame(get_query_result('queries/price.sql'))
```

```{python}
df_vol = pd.DataFrame(get_query_result('queries/vol.sql'))
```
...
```{python}
df_sv = pd.DataFrame(get_query_result('queries/stableCoin_vol.sql'))
```
