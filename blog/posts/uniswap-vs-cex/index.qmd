...
df_rest = pd.DataFrame({'parentName': [f'Other {rest_count} DEXs'], 'tvl': [df_rest['tvl'].sum()]})
...
df_rest = pd.DataFrame({'name': [f'Other {rest_count} CEXs'], 'tvl': [df_rest['tvl'].sum()]})
...
df_concat = df_concat[
    (df_concat['date'] >= '2023-01-01') & (df_concat['date'] <= '2023-07-04')
]

df_concat.rename(columns={'date': 'Date', 'totalLiquidityUSD': 'Total Assets in USD'}, inplace=True)

fig = px.line(
    df_concat, x='Date', y='Total Assets in USD', title='Total Assets YTD 2023',
    facet_row='Exchange',
    height=800
)
...
```{python}
df_top3_cex = df_cex.sort_values('tvl', ascending=False).head(3)
...
for df, ex in zip([pd.DataFrame()]*4, exs):
    ...
    df = pd.concat([df[['name']], df_concat], axis=1)
    df = df.melt(id_vars=['name'], value_vars=list(df_concat.columns), var_name='chain', value_name='tvl')
