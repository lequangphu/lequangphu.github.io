...
```{python}
df_uniswap_v2 = pd.json_normalize(r.json()['result']['rows'])
df_uniswap_v2.rename(columns={'amount_usd': 'tvl', 'symbol': 'token'}, inplace=True)
df_uniswap_v2 = df_uniswap_v2[['token', 'tvl']]
```

```{python}
r_v1 = requests.get(f'https://api.dune.com/api/v1/query/2686308/results?api_key={api_key}')
```

```{python}
df_uniswap_v1 = pd.json_normalize(r_v1.json()['result']['rows'])
df_uniswap_v1.rename(columns={'amount_usd': 'tvl', 'symbol': 'token'}, inplace=True)
df_uniswap_v1 = df_uniswap_v1[['token', 'tvl']]
```

```{python}
df_uniswap = pd.concat([df_uniswap_v1, df_uniswap_v2, df_uniswap_v3[['token', 'tvl']]], ignore_index=True)
...
<iframe src="https://dune.com/embeds/2685347/4465685" width="100%" height="400"></iframe>
