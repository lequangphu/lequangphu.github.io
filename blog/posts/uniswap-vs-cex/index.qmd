...
for df, ex in zip([pd.DataFrame()]*4, exs):
    ...
    df = df.melt(value_vars=list(df.columns), var_name='tokens', value_name='tvl')
    df_above = df[df['tvl'] / df['tvl'].sum() >= 0.01]
    df_rest = df[df['tvl'] / df['tvl'].sum() < 0.01]
    rest_count = df_rest['tvl'].count()
    df_rest = pd.DataFrame({'tokens': [f'{rest_count} Others'], 'tvl': [df_rest['tvl'].sum()]})
    df = pd.concat([df_above, df_rest], ignore_index=True)
    df['name'] = ex
    dfs.append(df)
...
```{python}
fig = px.pie(
    df_uniswap_v3, names='tokens', values='tvl', title='Uniswap V3 Assets by Token',
    hover_data={'tvl': ':.3s'}
)
...
#| column: screen-inset
fig = px.pie(
    pd.concat(df_cexs, ignore_index=True), names='tokens', values='tvl', title='CEX Assets by Token',
    facet_col='name',
    hover_data={'tvl': ':.3s'}
)
